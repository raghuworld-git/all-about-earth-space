...
export const getFormattedTop5UpcomingList = (data = []) => {
    ...
    let dataArray = data.map((item) => {
        const { name, net, id, slug, status, launch_service_provider, pad } = item;
        ...
        return { name, id, slug, location: pad.location?.name, agency: launch_service_provider.name, statusFull: `${status.name} (${status.abbrev})`, statusFullColor: statusColors[0], shortStatus: status.abbrev, shortStatusColor: statusColors[1], fromattedNet, originalNet: net }
    })
